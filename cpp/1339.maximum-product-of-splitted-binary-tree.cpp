...
public:
    int mod = 1e9+7;
    long long getSum(TreeNode* root, unordered_map<TreeNode*, long long>& dp)
    {
        if(root==nullptr) return 0;
        int left = getSum(root->left, dp);
        int right= getSum(root->right, dp);
        return dp[root]=(left+right+root->val);
    }
    int maxProduct(TreeNode* root) {
        unordered_map<TreeNode*, long long> dp;
        long long total = getSum(root, dp);
        long long res = 0;
        for(auto& [node, sum] : dp)
        {
